...
Plugin URI:  https://benjihughes.co.uk
Description: Uses lazysizes.js to lazyload images in post content.
Version:     1.0.3
Author:      Benji Hughes
...
class LazyLoadImages {
	...
	public function lazyload_add_admin_menu() {
		add_options_page( 'LazyLoad Images', 'LazyLoad Images', 'manage_options', 'lazyload_image', array($this, 'lazyload_options_page') );
	}
	...
	public function lazyload_checkbox_loadscript_render() {
        ...
        <input type="checkbox" name="lazyload_settings[lazyload_checkbox_loadscript]" <?php echo checked( $this->options['lazyload_checkbox_loadscript'], 1 ); ?> value="1">
	...
	public function lazyload_text_classname_render() {
        ...
        <input type="text" name="lazyload_settings[lazyload_text_classname]" value="<?php echo $this->options['lazyload_text_classname']; ?>">
	...
	public function convert_to_lazyload( $content ) {
		...
		// Capture all images with the specified class.
		...
		foreach ( $matches as $val ) {
			...
			$replace = str_replace( 'src=', 'data-src=', $val );
